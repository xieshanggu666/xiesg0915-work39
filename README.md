...
整改往往只涉及个别单体 / 专业 / 核查项。`batch` 支持圈定本次**复查范围**，
台账只对**真正成功覆盖**的问题自动销项，避免“没扫到”被误判成“已整改”：

```bash
# 只复查指定单体（可重复；按单体名精确匹配文件名去后缀）
python -m ifc_audit.cli batch 模型目录/ --project X \
    --recheck-unit 1号楼 --recheck-unit 2号楼
# 只复查机电专业（可重复 arch/struct/mep）
python -m ifc_audit.cli batch 机电模型/ --project X --recheck-discipline mep
# 只复查指定核查项（可重复 / 组合）：
#   wall=墙体闭合（墙自由端/墙段缺口/房间围护缺口/房间无几何）
#   duplicate=重复构件  room=房间净面积  opening=门窗规格
#   也可给具体 kind，如 wall_free_end
python -m ifc_audit.cli batch 模型目录/ --project X --recheck-kind wall
# 三个维度可任意组合（取交集）
python -m ifc_audit.cli batch 模型目录/ --project X \
    --recheck-unit 1号楼 --recheck-discipline arch --recheck-kind opening
```

范围与覆盖规则：

- **申请范围留痕**：每次合入在台账记一条复查运行（`scan_runs`），保存
  申请的单体 / 专业 / 核查项、**实际成功扫描的单体**、扫描失败文件
  （单体 / 专业 / 文件 / 错误）、每份模型的**版本标识**（文件大小 +
  内容 SHA-1，≤256MB 整文件哈希、超大模型抽样哈希；提模后必然变化）；
  工单上记最近覆盖结论与销项时的模型版本；
- **仅成功覆盖才自动销项**：范围内、依赖模型全部扫描成功且未再检出的工单，
  按原规则自动复核通过 / 标已消除，并把模型版本写入工单与流转记录；
- **范围外保留**：不在复查单体 / 专业 / 核查项内的工单**状态原样保留**
  （标 `out_of_scope`），不会因为本次没扫而被误销项；
- **扫描失败保留**：范围内但对应 IFC 解析 / 核查失败（`UnitResult.ok=False`、
  协同专业模型失败）的工单**状态原样保留**（标 `scan_failed`），写
  `scan_failed` 流转记录（同批次幂等，不重复刷通知）；
- **依赖配对判定**：协同（碰撞 / 洞口）工单跨专业，只提交机电模型时
  结构 / 建筑侧依赖未覆盖，相关协同工单按范围外保留——必须把涉及专业模型
  都纳入本次扫描才会自动销项；
- 不给任何 `--recheck-*` 即全量复查，历史行为不变。

通知与门禁随覆盖结果联动：

- 局部复查 / 扫描失败导致覆盖不完整时，向项目协调与各专业负责人投递
  **「复查覆盖不完整」**通知（列失败模型、范围外与失败保留工单数；同批次去重）；
- 闭环门禁新增两条覆盖度判定：
  - `default`：未覆盖活动工单为**告警项（不阻断放行）**，控制台 / Excel /
    JSON 中列出数量；扫描失败不阻断但会提示；
  - `strict`：未覆盖活动工单必须为 0、存在扫描失败模型即**阻断放行**，
    防止不完整复查被当成全部闭环通过竣工门禁；
  - `loose` / `none` 不检查覆盖度。
- 闭环 Excel 新增「复查记录」表（范围 / 成功失败单体 / 模型版本 / 各类销项
  与保留计数 / 未覆盖工单），「工单台账」表与工单 CSV 增加**最近覆盖、
  覆盖批次/时间、复查模型版本**列，闭环概览展示最近一次复查摘要。

历史台账兼容：旧台账（schema v1、无复查字段）可直接加载，`runs` 为空、
汇总中最近复查为 `None`；下一次扫描后按新结构保存（schema v2），
其中成功覆盖且消失的老工单照常自动销项。

#### 工单的跨批次稳定标识（批次范围 / 同名文件不漂移）

局部复查、跨目录同名文件、问题整改后重新编号等批次组成变化，都不能让同一问题
“换身份”，否则会出现重复建单、无法销项或同名模型互相覆盖复查状态。系统在显示名
之外维护与批次组成无关、且能**区分不同目录同名模型**的**稳定标识**：

- **单体稳定键 `unit_key`**：模型相对**模型根锚点**的 POSIX 相对路径去后缀，
  如 `A区/楼A`、`B区/楼A`、`一期/样例-建筑模型`。不同目录下的同名文件因此得到
  不同的键，工单指纹互不共享、复查状态互不覆盖；`unit`（显示名）在跨目录同名
  并存时会带父目录消歧（`A区-楼A`）、随批次组成变化，**仅用于展示**，不进指纹。
- **模型根锚点（model root）**：相对路径需要固定锚点。按
  ① 显式 `--model-root` → ② 项目历史配置 `<history>/<项目>/model_root.json`
  中固化的锚点 → ③ 本批全部文件的最长公共父目录 的优先级确定；**首次自动确定后
  持久化**，后续局部复查（只扫一个子目录）也沿用同一锚点，键不会退化成纯文件名。
  搬迁模型目录后用 `--model-root` 显式指定即可保持一致。
- **批量审查（audit）工单指纹**：`audit|核查项|unit_key|楼层|排序GlobalId|
  位置50mm网格|量化指标`，**不含每次扫描重新编号的** `issue_id`
  （GAP/END/ENC 序号随检测顺序变化），也不含消歧显示名。
- **多专业协同（coord）指纹**：沿用 `核查项|构件GlobalId`；协同工单的每条构件
  引用同样带 `unit_key`，按单体的覆盖判定 / 建筑侧回写用稳定键分组。
- **跨目录同名的历史迁移**：旧台账 `unit_key` 是纯文件名（上一版口径）时，弱键
  按 basename 尾段兼容，但当出现多个同名候选时只接受**稳定键完全相等**的那一个，
  避免 `A区/楼A` 误并到 `B区/楼A`；构件 GlobalId + 楼层 + 位置网格 + 指标仍逐位
  区分，复制模型（同 GlobalId、不同位置）也不会并单。
- 台账提供**旧指纹别名迁移**：历史工单（指纹含旧 `issue_id` + 消歧显示名）在
  新批次命中时，先按旧精确指纹、再按物理弱键（核查项 + 单体 + 楼层 + 构件 +
  位置网格 + 指标，要求唯一候选）回查；命中即把工单**主键迁移到新指纹并把旧指纹
  记入 `fingerprint_aliases`**，写 `fp_migrate` 流转记录，状态、责任人、整改 /
  复核记录全部保留；同构件不同楼层 / 不同位置 / 不同量化指标的问题仍严格区分，
  不会误并。
- 趋势对比（单体新增 / 缺失）也改用稳定键匹配，跨目录同名文件的消歧前缀变化不再
  被误判成“单体换了一批”。

`--recheck-unit` 同时接受显示名与稳定键（相对路径）；批次 JSON / 快照记录
`model_root` 与每个单体的 `unit_key`，工单 Excel / CSV 增加「单体标识」列。
