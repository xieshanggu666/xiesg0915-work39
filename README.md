...
#### 工单的跨批次稳定标识（批次范围变化不漂移）

局部复查、跨目录同名文件、问题整改后重新编号等批次组成变化，都不能让同一问题
“换身份”，否则会出现重复建单或无法销项。系统在显示名之外维护与批次组成无关的
**稳定标识**：

- **单体**：`unit_key` = IFC 文件名去后缀（不含目录），跨批次恒定；
  `unit`（显示名）在跨目录同名文件并存时会带父目录消歧、随批次组成变化，
  仅用于展示，不进指纹。
- **批量审查（audit）工单指纹**：`audit|核查项|unit_key|楼层|排序GlobalId|
  位置50mm网格|量化指标`，**不含每次扫描重新编号的** `issue_id`
  （GAP/END/ENC 序号随检测顺序变化），也不含消歧显示名。
- **多专业协同（coord）指纹**：沿用 `核查项|构件GlobalId`，本就与单体名无关。
